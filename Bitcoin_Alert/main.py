...
photo = ImageTk.PhotoImage(Image.open("bitcoincasino.png"))
...
def say_hi():
    ...
    lbl1a = Label(
        root, text="Requirements :", bg="black", fg="white", font="Times 18")
    lbl1a.config(anchor=CENTER)
    lbl1a.pack(pady=5)
    lbl1b = Label(
        root, text="1 - IFTTT Account.",
        bg="black",
        fg="white",
        font="Times 18")
    ...
    lbl1b.pack(pady=5)
    lbl1c = Label(
        root, text="2 - Applet created with webhooks.",
        bg="black",
        fg="white",
        font="Times 18")
    ...
    lbl1c.pack(pady=5)
    lbl1d = Label(
        root, text="3 - Two event for emergency "
        " alert and normal alerts.",
        bg="black", fg="white", font="Times 18")
    lbl1d.config(anchor=CENTER)
    lbl1d.pack(pady=5)
    lbl1e = Label(
        root, text="4 - Suggested Message Format is\n    Normal Alert "
        " : {Bitcoin Alerts : <br> value1 }\n    Emergency Alert : "
        " {Emergency Alert : <br> value1}",
        bg="black", fg="white", font="Times 18")
...
check_requirement = Button(
    root, text="Check Requirments", command=say_hi)
...
def say_country():
    ...
    lbl2 = Label(
        root, text="Available Country :\nINR - INDIA\nUSD - UNITED STATES "
        " OF AMERICA\nJPY - JAPAN\nEUR - EUROPE\n AND Still more !",
        bg="black", fg="white", font="Times 18", pady=10)
...
countries = Button(
    root, text="Countries", command=say_country)
...
def main_func():
    ...
    start.destroy()
    country = Label(
        root, text='Enter the country code to setup the alerts',
        bg="black", fg="white", font="Times 18", pady=10)
    country.pack(side=TOP, pady=10)
    textExample = Text(root, height=1, width=12)
    textExample.pack(side=TOP, pady=5)
    countryCode_button = Button(
        root, text="Enter", command=retrieve_input)
    countryCode_button.pack(side=TOP, pady=10)
    html_label = HTMLLabel(
        root, html='<a href="https://blockchain.info/ticker"'
...
start = Button(
    root, text="Start", command=main_func)
start.pack(side=TOP, pady=10)


# *******************
# Function to remove quote and following labels
# and text(this comes after starting the notifier app function)
def retrieve_input():
...
def retrieve_inputForThresh_input():
    ...
    threshinput.destroy()
    event1 = Label(
        root, text='Enter the event name that you created in IFTTT',
        bg="black", fg="white", font="Times 18")
...
def retrieve_event3input():
    ...
    timer = Label(
        root, text='Enter interval time for alerts\n NOTE use '
        ' 720 for 1hr interval',
        bg="black", fg="white", font="Times 18")
...
def retrieve_timer():
    ...
    timer.destroy()
    serverNote = Label(
        root, text='Server is synchronizing '
        ' your data for notification !!!',
        bg="white", fg="black", font="Times 18")
    serverNote.pack(side=TOP, pady=10)
    iftttLinked = Label(
        root, text='IFTTT is linked.',
        bg="black", fg="white", font="Times 18")
    iftttLinked.pack(side=TOP, pady=5)
    exitButton = Button(
        root, text="Click here to fire the notification", command=root.destroy)
    exitButton.pack(side=TOP, pady=10)

    arr = [countryCode, eventinput1, threshinput1,
           eventinput2, eventinput3, eventinput4]
    # return arr
