...
def say_hi():
    ...
    lbl1a=Label(root, text = "Requirements :", bg="white", fg="black", font="Times 18")
    lbl1a.config(anchor=CENTER)
    lbl1a.pack(pady = 5)
    lbl1b=Label(root, text = "1 - IFTTT Account.", bg="white", fg="black", font="Times 18")
    lbl1b.config(anchor=CENTER)
    lbl1b.pack(pady = 5)
    lbl1c=Label(root, text = "2 - Applet created with webhooks.",bg="white", fg="black", font="Times 18")
    lbl1c.config(anchor=CENTER)
    lbl1c.pack(pady = 5)
    lbl1d=Label(root, text = "3 - Two event for emergency alert and normal alerts.",bg="white", fg="black", font="Times 18")
    lbl1d.config(anchor=CENTER)
    lbl1d.pack(pady = 5)
    lbl1e=Label(root, text = "4 - Suggested Message Format is\n    Normal Alert : {Bitcoin Alerts : <br> value1 }\n    Emergency Alert : {Emergency Alert : <br> value1}",bg="white", fg="black", font="Times 18")
...
def say_country():
    ...
    lbl2=Label(root, text = "Available Country :\nINR - INDIA\nUSD - UNITED STATES OF AMERICA\nJPY - JAPAN\nEUR - EUROPE\n AND Still more !", bg="white", fg="black", font="Times 18", pady = 10)
...
def main_func():
    ...
    lbl2.destroy()
    countries.destroy()
    lbl3=Label(root, text = "---------------------------------------------------------------------------------\n---------------------------------------------------------------------------------\n---------------------------------------------------------------------------------\n---------------                                                 -----------------\n    “The future is ours to shape. I feel we are in a race that we need to\n       win.It’s a race between the growing power of the technology\n                 and the growing wisdom we need to manage it.”                         \n                                                                                            ~Max Tegmark                                    \n---------------------------------------------------------------------------------\n---------------------------------------------------------------------------------\n---------------------------------------------------------------------------------", bg="white", fg="maroon", font="Times 18", pady = 10)
    lbl3.config(anchor=CENTER)
    lbl3.pack(pady = 10)
    start.destroy()
    country = Label(root, text ='Enter the country code to setup the alerts',bg="white", fg="black", font="Times 18", pady = 10)
...
def retrieve_input():
    ...
    thresh = Label(root, text ='Enter Emergency Threshold',bg="white", fg="black", font="Times 18" )
...
def retrieve_inputForThresh_input():
    ...
    threshLimit.destroy()
    threshinput.destroy()
    event1 = Label(root, text ='Enter the event name that you created in IFTTT', bg="white", fg="black", font="Times 18" )
...
def retrieve_eventinput():
    ...
    event1.destroy()
    eventButton.destroy()
    event2 = Label(root, text ='Enter the Emergency threshold event Name', bg="white", fg="black", font="Times 18")
...
def retrieve_event2input():
    ...
    event3 = Label(root, text ='Enter the IFTTT token ,You Got from Webhooks', bg="white", fg="black", font="Times 18")
...
def retrieve_event3input():
    ...
    timer = Label(root, text ='Enter interval time for alerts\n NOTE use 720 for 1hr interval',bg="white", fg="black", font="Times 18")
...
def retrieve_timer():
    ...
    timerinput.destroy()
    timer.destroy()
    serverNote = Label(root, text ='Server is synchronizing your data for notification !!!', bg="white", fg="black", font="Times 18")
    serverNote.pack(side = TOP, pady = 10)
    # sleep(5)
    iftttLinked = Label(root, text ='IFTTT is linked.', bg="white", fg="black", font="Times 18")
